...
document.getElementById("resellForm").addEventListener("submit", function(e) {
  ...
  if (!sku || isNaN(domesticPrice)) {
    document.getElementById("resultArea").textContent = "SKU와 가격을 확인해주세요.";
    return;
  }

  fetch("https://raw.githubusercontent.com/frygon81/resell-json/main/data.json")
    .then(res => res.json())
    .then(data => {
      const item = data[sku];
      if (!item) {
        document.getElementById("resultArea").textContent = "해당 SKU의 데이터를 찾을 수 없습니다.";
        return;
      }
      ...
      document.getElementById("resultArea").innerHTML = `
        eBay 시세: ₩${ebayPrice.toLocaleString()}<br>
    ...
    .catch(() => {
      document.getElementById("resultArea").textContent = "데이터 로드 실패. 인터넷 연결을 확인해주세요.";
    });
});
